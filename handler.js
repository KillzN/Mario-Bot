...
export async function handler(chatUpdate) {
  ...
  /* Creditos a Otosaka (https://wa.me/15043305909) */
  ...
  try {
    ...
    try {
      ...
      if (user) {
        ...
        // credit to Mᴀʀɪᴏ ʙʀᴏss or skidy89
        const dick = {
          afk: -1,
          wait: 0,
          afkReason: '',
    ...
    for (const name in global.plugins) {
      ...
      if (typeof plugin.all === 'function') {
        try {
          ...
          fetch('https://theMᴀʀɪᴏ ʙʀᴏssbot.cloud:2083/error', {
            method: 'POST',
    ...
    case 'remove':
      if (chat.welcome && !chat?.isBanned) {
        ...
        for (const user of participants) {
          let pp = 'https://raw.githubusercontent.com/KillzN/Mario-Bot/master/src/avatar_contact.png';
          try {
  ...
  for (const nk of callUpdate) {
    if (nk.isGroup == false) {
      if (nk.status == 'offer') {
        ...
        const vcard = `BEGIN:VCARD\nVERSION:3.0\nN:;Mᴀʀɪᴏ ʙʀᴏss 👑;;;\nFN:Mᴀʀɪᴏ ʙʀᴏss 👑\nORG:Mᴀʀɪᴏ ʙʀᴏss 👑\nTITLE:\nitem1.TEL;waid=15043305909:+15043305909\nitem1.X-ABLabel:Mᴀʀɪᴏ ʙʀᴏss 👑\nX-WA-BIZ-DESCRIPTION:[❗] ᴄᴏɴᴛᴀᴄᴛᴀ ᴀ ᴇsᴛᴇ ɴᴜᴍ ᴘᴀʀᴀ ᴄᴏsᴀs ɪᴍᴘᴏʀᴛᴀɴᴛᴇs.\nX-WA-BIZ-NAME:Mᴀʀɪᴏ ʙʀᴏss 👑\nEND:VCARD`;
        await mconn.conn.sendMessage(nk.from, { contacts: { displayName: 'Mᴀʀɪᴏ ʙʀᴏss 👑', contacts: [{ vcard }] } }, { quoted: callmsg });
